...
- Tamper-evident execution event chaining: every recorded execution event now carries `prev_hash` and `event_hash` (SHA-256 over canonical event JSON, genesis-linked), the persisted `integrity` block records the chain head as `event_chain_head`, and `verify_execution_event_chain(events)` / `verify_persisted_event_chain(path)` validate the timeline. Legacy unhashed events are accepted only as a pre-chain prefix and are never backfilled.
- Opt-in versioned snapshot history via `ProjectState.snapshot_history_limit` (default `0`): each save appends a full payload copy (JSON backend: `<state_file>.history/` directory; SQLite backend: `project_state_history` table) with retention-aware pruning that never touches the live event chain. New public helpers `list_state_snapshots(path)` and `load_state_snapshot(path, version)`.
- Advisory POSIX file locking around state persistence: `save()` and `load()` now take an `fcntl` lock on a `<state_file>.lock` companion (exclusive/shared), exposed as `state_file_lock(path)`; the documented concurrency contract remains single-writer.
- Complete provider-call history: each task now persists an append-only `provider_calls` list covering every provider call across retries (target, outcome, duration, attempt counters, per-agent call index, recorded-at timestamp), with `last_provider_call` retained as a derived view. New evidence configuration on `KYCortexConfig`: `evidence_sanitization_mode` (`strict` default, or `audit` to preserve redacted error text and fallback model names in call histories) and opt-in `evidence_capture_prompts` with `evidence_prompt_capture_max_chars` size limits for recording redacted prompts and raw provider responses per call.
- State integrity digests: every `ProjectState.save()` now embeds a SHA-256 `integrity` block over the canonical persisted payload and writes a `<state_file>.sha256` sidecar; `verify_persisted_state_integrity(path)` recomputes and checks the digest. Artifact persistence now maintains an `artifacts_manifest.json` (SHA-256, size, name, type per persisted artifact) at the output-directory root.
- Report provenance in the internal observability HTML shell: a dedicated provenance section with generation timestamp (UTC), tool version, source state file, store kind, schema version, and the source-state SHA-256, plus an internal-sensitivity label, redaction notice, and an explicit disclaimer that the report is operational telemetry rather than a certified audit record. Share-link copy no longer advertises the report as freely shareable.
- Execution provenance foundation in persisted workflow state: `ProjectState.run_identity` (run id, OS user, hostname, pid, package/Python versions, platform, and clock metadata) captured on workflow start, monotonic `sequence` numbers on every execution event, and per-task `execution_mode` provenance (`provider`, `deterministic`, or `manual_override`) so scripted runs stay distinguishable from provider-backed runs. Persisted schema advanced to version 2 with automatic migration and positional sequence backfill for older state files.
...
This beta release extends `1.0.13b1` with a safer default completion budget for `release-user-smoke` and an explicit override for future canary tuning.
...
- CI typecheck hotfix: `extract_required_fields()` now keeps optional literal-sequence lookups on dedicated local bindings, clearing the post-publication `mypy` regression on both Python 3.10 and 3.12 without changing runtime behavior.
- CI coverage-gate hotfix: `skip_superseded_pending_repairs()` now keeps pending repairs that are still justified by an unresolved `source_failure_task_id`, and the affected orchestrator regression now expects nested constructor payload normalization to preserve existing keys while adding required ones.
