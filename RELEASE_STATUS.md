...
- The replacement beta canary is active at `docs/canary-evidence/1e68a8b/`; smoke01 through smoke09 closed `25/25` accepted with zero incidents and zero rollbacks, and the early window has now crossed the `25-workflows` checkpoint cleanly.
...
- The replacement candidate `1e68a8b` / `v1.0.13b2` reached the 25-workflows checkpoint at `2026-05-11T11:47:20Z` with cumulative `25/25` accepted workflows and no incident.
- No release workflow is currently in progress.

## Next Release-Facing Action

1. Continue the replacement beta canary through the `50-workflows` checkpoint for candidate `1e68a8b` / `v1.0.13b2`.
