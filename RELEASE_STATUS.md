...
1. Continue canary observation until the next UTC-day review checkpoint for candidate `1e68a8b` / `v1.0.13b2`, when `daily-review day-3` becomes the next repository-owned packet.
2. Maintain repository-owned checkpoint packets, including the retained retryable-incident record from `daily-review day-1` and the clean `daily-review day-2` continuation, until the `7`-day/`100`-workflow minimum window closes.
