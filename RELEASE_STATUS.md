...
- Package version in `pyproject.toml`: `1.0.13b1`
 - Package version in `pyproject.toml`: `1.0.13b2.dev0`
...
- The current branch has reopened on development version `1.0.13b2.dev0` while published `v1.0.13b1` remains the latest released beta baseline.
...
- The reopened `1.0.13b2.dev0` line is now carrying the first follow-on fix work for the held beta path.
- No release workflow is currently in progress.

## Next Release-Facing Action

1. Root-cause the retained `code_validation` incident on `c17c749` and validate the follow-on `release-user-smoke` budget fix on `1.0.13b2.dev0`.
2. If the held candidate still cannot be explained cleanly, prepare a replacement beta candidate instead of resuming the held bundle.
