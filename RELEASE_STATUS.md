...
- The replacement beta canary is active at `docs/canary-evidence/1e68a8b/`; smoke01 through smoke04 closed `10/10` accepted with zero incidents and zero rollbacks, and the early window has now crossed the `10-workflows` checkpoint cleanly.
...
- The replacement candidate `1e68a8b` / `v1.0.13b2` reached the 10-workflows checkpoint at `2026-05-11T11:26:33Z` with cumulative `10/10` accepted workflows and no incident.
- No release workflow is currently in progress.

## Next Release-Facing Action

1. Continue the replacement beta canary through the `25-workflows` checkpoint for candidate `1e68a8b` / `v1.0.13b2`.
