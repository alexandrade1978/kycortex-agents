...
Status: live host pinned and first checkpoint recorded; canary window remains open
...
| Canary host | Native maintainer-operated Linux host `alex-kycortex` | Preflight provider health captured on `Linux 6.17.0-20-generic x86_64 GNU/Linux`, Python `3.12.3`, with the maintainer provider env file present before traffic | confirmed |
| Provider set | OpenAI `gpt-4o-mini`, Anthropic `claude-haiku-4-5-20251001`, and Ollama `qwen2.5-coder:7b` at `http://127.0.0.1:11434` | Preflight provider health was healthy for all three providers at `2026-04-13T02:34:45.948542+00:00`; the first accepted workflow exercised OpenAI `gpt-4o-mini` cleanly | confirmed preflight / partially exercised |
...
| Telemetry access | Operator can export both `snapshot()` and `internal_runtime_telemetry()` from the same persisted state | The first accepted OpenAI checkpoint was exported from repository-owned state and validated against the generated artifact | confirmed |
...
Expansion is intentionally held at the smallest practical subset after one clean accepted workflow. The next expansion requires updated provider health and checkpoint evidence.