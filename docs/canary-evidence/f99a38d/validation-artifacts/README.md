...
- the active canary window is still open, so this directory currently stores the preflight packet, three expansion health refreshes, the clean 10-workflow, 25-workflow, and 50-workflow checkpoints, and the first same-day daily review rather than a completed close-out set
