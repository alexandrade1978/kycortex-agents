...
def _print_provider_health_summary(provider_health_summary: Mapping[str, Mapping[str, Any]]) -> None:
    ...
    for index, provider_name in enumerate(sorted(provider_health_summary), start=1):
        ...
        print(
            ...
            f"active_checks={_presence_label(health.get('has_active_checks'))}"
        )
...
def main() -> None:
    ...
    print("\nWorkflow telemetry:")
    print(f"multiple_tasks={_presence_label(workflow_telemetry['has_multiple_tasks'])}")
    ...
    print(f"multiple_observed_providers={_presence_label(workflow_telemetry['has_multiple_observed_providers'])}")
    print(f"multiple_final_providers={_presence_label(workflow_telemetry['has_multiple_final_providers'])}")
    print(f"attempts_present={_presence_label(workflow_telemetry['has_attempts'])}")
    print(f"retry_attempts_present={_presence_label(workflow_telemetry['has_retry_attempts'])}")
    print("\nWorkflow progress summary:")
    print(f"pending_tasks_present={_presence_label(progress_summary['has_pending_tasks'])}")
    print(f"running_tasks_present={_presence_label(progress_summary['has_running_tasks'])}")
    print(f"runnable_tasks_present={_presence_label(progress_summary['has_runnable_tasks'])}")
    print(f"blocked_tasks_present={_presence_label(progress_summary['has_blocked_tasks'])}")
    print(f"terminal_tasks_present={_presence_label(progress_summary['has_terminal_tasks'])}")
