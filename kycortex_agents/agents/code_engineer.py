...
If you use @dataclass anywhere in the module, import dataclass explicitly from dataclasses so the module imports cleanly. Do not leave @dataclass in the file while importing only field or other dataclasses helpers.
...
If import validation reports NameError that dataclass is not defined, add `from dataclasses import dataclass` or remove every @dataclass decorator before finalizing. Do not leave decorator usage without the matching import.
...
If you use @dataclass anywhere in the module, import dataclass explicitly from dataclasses so the module imports cleanly. Do not leave @dataclass in the file while importing only field or other dataclasses helpers.
If you use dataclasses.field(...) or field(default_factory=...) anywhere in the module, import field explicitly from dataclasses so the module imports cleanly.
If import validation reports NameError that dataclass is not defined, add `from dataclasses import dataclass` or remove every @dataclass decorator before you finalize. Do not leave decorator usage without the matching import.
...
- if you used @dataclass anywhere in the module, you imported dataclass explicitly from dataclasses so the module imports cleanly
