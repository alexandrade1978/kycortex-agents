...
def test_execute_records_retry_attempt_metadata_for_transient_provider_recovery(monkeypatch):
    ...
    assert result.metadata["provider_call"]["attempt_history"][0]["has_error_type"] is True
    assert "error_type" not in result.metadata["provider_call"]["attempt_history"][0]
...
def test_execute_falls_back_to_secondary_provider_after_primary_health_check_failure(monkeypatch):
    ...
    assert "has_last_error_type" not in metadata["provider_health"]["openai"]
    assert "last_error_type" not in metadata["provider_health"]["openai"]
    assert metadata["provider_health"]["openai"]["last_health_check"]["has_error_type"] is True
    assert "error_type" not in metadata["provider_health"]["openai"]["last_health_check"]
    assert metadata["provider_health"]["openai"]["last_health_check"]["has_error_message"] is True
