...
def test_snapshot_round_trip_preserves_mixed_task_state_integrity(tmp_path, state_filename):
    ...
    assert "has_error_category" not in review_result.details
    ...
    assert "last_error_category" not in review_result.details
    ...
    assert workflow_telemetry == {
        ...
        "has_attempts": False,
        "has_retry_attempts": False,
        "duration_ms": {"has_samples": True, "has_multiple_samples": True},
        "usage": {"completion_tokens": True, "prompt_tokens": True, "total_tokens": True},
        "fallback_summary": {
            "has_multiple_tasks": False,
            "has_entries": False,
